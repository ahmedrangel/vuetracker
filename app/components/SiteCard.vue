...
<template>
  <div class="flex flex-col bg-gray-200 dark:bg-gray-900 rounded overflow-hidden">
    <div class="relative h-[100px] sm:h-[160px] md:h-[140px] lg:h-[140px] xl:h-[180px]">
      <img v-if="site.ogImage" :src="site.ogImage" class="absolute object-cover h-full w-full" :title="site.title || normalizeSITE(site.url)" loading="lazy" @error="hideImageOnError">
    ...
    <div class="flex flex-wrap gap-2 items-center justify-between p-2">
      <NuxtLink :to="`/${normalizeSITE(site.url)}`" class="hover:underline truncate">
        <div class="flex gap-1 items-center">
          <img :src="findFavicon(site.icons) || `https://${site.hostname}/favicon.ico`" class="max-w-4 max-h-4 min-w-4 min-h-4" loading="lazy" @error="hideImageOnError">
